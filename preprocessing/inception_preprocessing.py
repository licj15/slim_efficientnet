...
def _mean_image_norm(image, means, var):
  if image.get_shape().ndims != 3:
    raise ValueError('Input must be of size [height, width, C>0]')
  num_channels = image.get_shape().as_list()[-1]
  if len(means) != num_channels:
    raise ValueError('len(means) must match the number of channels')
  if len(var) != num_channels:
    raise ValueError('len(var) must match the number of channels')
...
def preprocess_for_train(image,
  ...
  with tf.name_scope(scope, 'distort_image', [image, height, width, bbox]):
    ...
    means = [0.451, 0.451, 0.451]
    var = [0.226, 0.226, 0.226]
    distorted_image = _mean_image_norm(distorted_image, means, var)
...
def preprocess_for_eval(image,
  ...
  with tf.name_scope(scope, 'eval_image', [image, height, width]):
    ...
    # image = tf.multiply(image, 128.0)
    # means = [0.485, 0.456, 0.406]
    # var = [0.229, 0.224, 0.225]
    means = [0.451, 0.451, 0.451]
    var = [0.226, 0.226, 0.226]
    image = _mean_image_norm(image, means, var)
    return image
